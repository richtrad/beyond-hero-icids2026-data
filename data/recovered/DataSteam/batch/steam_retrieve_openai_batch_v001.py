...
# -*- coding: utf-8 -*-
"""
steam_retrieve_openai_batch_v001.py

Check a Batch job, download output/error files if available, and merge structured
protagonist results back into the broad pool CSV.
...
Inputs:
  steam_bulk_outputs/steam_protagonist_batch_id_v001.txt
  steam_bulk_outputs/steam_archetype_broad_pool_for_coding.csv

Outputs:
  steam_bulk_outputs/steam_protagonist_batch_output_v001.jsonl
  steam_bulk_outputs/steam_protagonist_batch_errors_v001.jsonl
  steam_bulk_outputs/steam_archetype_broad_pool_with_protagonists_v001.csv
"""
...
BATCH_ID_FILE = Path("steam_bulk_outputs/steam_protagonist_batch_id_v001.txt")
BASE_CSV = Path("steam_bulk_outputs/steam_archetype_broad_pool_for_coding.csv")
OUT_JSONL = Path("steam_bulk_outputs/steam_protagonist_batch_output_v001.jsonl")
OUT_ERRORS = Path("steam_bulk_outputs/steam_protagonist_batch_errors_v001.jsonl")
OUT_MERGED = Path("steam_bulk_outputs/steam_archetype_broad_pool_with_protagonists_v001.csv")
OUT_STATUS = Path("steam_bulk_outputs/steam_protagonist_batch_status_v001.json")


def response_text_from_body(body: Dict[str, Any]) -> Optional[str]:
    # Common convenience field in Responses API objects.
    if isinstance(body.get("output_text"), str):
        return body["output_text"]

    # Robust extraction from output[].content[].
...
def main() -> None:
    ...
    print(f"Parsed structured results: {len(results)} rows")
    ...
    # Prefix model result columns to avoid overwriting manual coding columns.
    ...
    prefixed = prefixed.rename(columns={
        c: f"llm_{c}" for c in prefixed.columns if c != "appid"
    })

    merged = base.merge(prefixed, on="appid", how="left")
    merged.to_csv(OUT_MERGED, index=False, encoding="utf-8-sig")

    print(f"Saved merged CSV: {OUT_MERGED} ({len(merged)} rows)")
    print("Suggested next: inspect rows with llm_needs_manual_review == True.")
